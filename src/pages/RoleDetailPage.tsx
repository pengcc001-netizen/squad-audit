  ...
  return (
    <div className="fade-in" style={{ maxWidth: 680, margin: '0 auto' }}>
      <Helmet>
        ...
        <script type="application/ld+json">{JSON.stringify({ "@context": "https://schema.org", "@type": "Article", "headline": `${role.name} — Squad Role`, "description": role.tagline, "url": url, "author": { "@type": "Person", "name": "Marcus Chen" }, "publisher": { "@type": "Organization", "name": "Squad Audit" } })}</script>
      </Helmet>
      <div style={{ marginBottom: 24 }}><Link to="/roles" style={{ color: 'var(--text-faint)', fontSize: 12, fontFamily: 'var(--font-mono)' }}>← All Roles</Link></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        <div className="audit-card" style={{ padding: '24px 28px' }}>
          ...
          {role.strengths.map((s, i) => <div key={i} style={{ fontSize: 14, color: 'var(--text)', padding: '4px 0', fontFamily: 'var(--font-body)' }}><span style={{ color: 'var(--success)', marginRight: 8 }}>+</span>{s}</div>)}
        </div>
        <div className="audit-card" style={{ padding: '24px 28px' }}>
          <div className="eyebrow" style={{ marginBottom: 8, color: 'var(--danger)' }}>Weaknesses</div>
          {role.weaknesses.map((w, i) => <div key={i} style={{ fontSize: 14, color: 'var(--text)', padding: '4px 0', fontFamily: 'var(--font-body)' }}><span style={{ color: 'var(--danger)', marginRight: 8 }}>−</span>{w}</div>)}
        </div>
      </div>
      <Ad />
      {/* Compatible roles */}
            ...
            {compatible.map(c => (
              <Link key={c.slug} to={`/roles/${c.slug}`} style={{ textDecoration: 'none' }}>
                <div className="audit-card" style={{ padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 14, color: 'var(--text)', fontFamily: 'var(--font-body)' }}>{c.name}</span>
                </div>
              </Link>
            ))}
          </div>
      ...
      <div style={{ marginTop: 32 }}><ShareButtons url={url} title={`${role.name} — ${role.tagline}`} /></div>
    </div>
  )
}
