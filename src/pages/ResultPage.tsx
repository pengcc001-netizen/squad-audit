...
import { computeAudit, decodeResult } from "../lib/squad";
  ...
  if (!decoded) {
    return (
      <div style={{ textAlign: "center", padding: "60px 0" }}>
        ...
        <Link to="/" className="btn btn-primary" style={{ marginTop: 16 }}>Take the Audit</Link>
      </div>
    );
  }

  const { role, scores } = computeAudit(decoded);
    ...
    <div className="fade-in" style={{ maxWidth: 680, margin: "0 auto" }}>
      ...
      <div className="audit-card" style={{ padding: "40px 32px", marginBottom: 32 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ fontSize: 11, letterSpacing: "0.3em", color: "var(--primary)", marginBottom: 8, fontFamily: "var(--font-mono)", textTransform: "uppercase" }}>Shared Audit Report</div>
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
            {role.responsibilities.map((r, i) => (
              <li key={i} style={{ fontSize: 14, color: "var(--text)", padding: "4px 0", fontFamily: "var(--font-body)" }}>
                <span style={{ color: "var(--secondary)", marginRight: 8 }}>▸</span>{r}
              </li>
            ))}
          </ul>
        </div>
        ...
        {compatibleRoles.length > 0 && (
          <div style={{ marginTop: 24, paddingTop: 24, borderTop: "1px solid var(--border)" }}>
            <div className="eyebrow" style={{ marginBottom: 12 }}>Compatible Roles</div>
